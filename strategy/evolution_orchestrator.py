...
def check_hard_constraints(trade_count: int, max_drawdown: float, win_rate: float) -> str:
...
def deflated_sharpe_p_value(
    ...
    expected_max_sr = math.sqrt(2 * math.log(num_experiments)) - (
        (euler_gamma + math.log(math.pi / 2)) / (2 * math.sqrt(2 * math.log(num_experiments)))
    )

    # Sharpe의 표준오차 (비정규성 보정)
    excess_kurtosis = kurtosis - 3.0
    sqrt_arg = 1.0 - skewness * observed_sharpe + (excess_kurtosis / 4.0) * observed_sharpe**2
    if sqrt_arg <= 0:
...
class EvolutionOrchestrator:
    ...
    async def run_session(self) -> EvolutionResult:
        ...
        if not validation.passed:
            ...
            return EvolutionResult(
                success=False,
                reason=f"validation_{validation.reason}",
                experiments_run=len(results),
    ...
    async def _phase_monitor(self) -> MonitorResult:
        ...
        profit_factor = abs(sum(wins)) / abs(sum(losses)) if losses and sum(losses) != 0 else 0.0
    ...
    async def _phase_diagnose(self, baseline: MonitorResult) -> list[str]:
        ...
        logger.info(
            "진단 결과: %s (PF=%.2f, SR=%.2f, MDD=%.1f%%, WR=%.1f%%)",
            weak,
    ...
    async def _phase_hypothesize(self, weak_areas: list[str]) -> list[ExperimentCandidate]:
        ...
        try:
            ...
            if patterns:
                ...
                for hyp in hypotheses[:5]:  # 최대 5개 가설
                    ...
                    for key in ("mr_sl_mult", "mr_tp_rr", "dca_sl_pct", "dca_tp_pct", "cutoff"):
    ...
    def _grid_strategy_candidates(self) -> list[ExperimentCandidate]:
        ...
        for sl_d, tp_d in [(-1.0, 0.5), (1.0, -0.3), (-2.0, 1.0), (1.0, 0.5)]:
            candidates.append(
                ExperimentCandidate(
                    changes={
                        "mr_sl_mult": base.mr_sl_mult + sl_d,
                        "mr_tp_rr": base.mr_tp_rr + tp_d,
                    },
                    source="grid_search",
    ...
    async def _backtest_strategy_change(
        ...
        if not strategies:
            return ExperimentResult(candidate=candidate, rejected="전략 매핑 실패")
        ...
        for strat in strategies:
            try:
                ...
                entries = self._optimizer.scan_entries(strat, candles_15m, candles_1h)
                if not entries:
                    continue
                opt_result = self._optimizer.replay_with_params(strat, params_dict, entries)
        ...
        if strat_count == 0:
            return ExperimentResult(candidate=candidate, rejected="백테스트 결과 없음")
    ...
    @staticmethod
    def _evolvable_to_strategy_params(params: EvolvableParams, strategy: str) -> dict[str, float]:
    ...
    async def _phase_apply(
        ...
        await self._notify(f"`/approve {change_id}` | `/reject {change_id}`")
